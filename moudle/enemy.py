...
class Enemy(Sprite):
    ...
    def init(self, randint):
        ...
        if self.type == 1:
            #敌飞船1速度
            self.speed_factor = self.game_starts.enemy_speed_factor1
            #敌飞船1生命
            self.life = self.game_starts.enemy1_life
            #敌飞船1分数
            self.score = self.game_starts.enemy1_score
            #敌飞船1图片
            for img in self.settings.enemy1_images:
                img = img.convert_alpha()
            self.images = self.settings.enemy1_images
            self.image = self.images[0]
            self.hit_change_time = None
            #敌飞船1爆炸图片
            self.down_images = self.settings.enemy1_down_images
            for down_image in self.down_images:
                down_image = down_image.convert_alpha()
            #敌飞船1被击中图片更改持续时间
            self.bomb_everyticktime = self.settings.enemy1_bomb_everyticktime
        elif self.type == 2:
            #敌飞船2速度
            self.speed_factor = self.game_starts.enemy_speed_factor2
            #敌飞船2生命
            self.life = self.game_starts.enemy2_life
            #敌飞船2分数
            self.score = self.game_starts.enemy2_score
            #敌飞船2图片
            ...
            for down_image in self.down_images:
                down_image = down_image.convert_alpha()
            self.bomb_everyticktime = self.settings.enemy2_bomb_everyticktime
        elif self.type == 3:
            #敌飞船3速度
            self.speed_factor = self.game_starts.enemy_speed_factor1
            #敌飞船3生命
            self.life = self.game_starts.enemy3_life
            #敌飞船3分数
            self.score = self.game_starts.enemy3_score
            #敌飞船3图片
            ...
            for down_image in self.down_images:
                down_image = down_image.convert_alpha()
            self.bomb_everyticktime = self.settings.enemy3_bomb_everyticktime
        #设置敌飞船移动动画每帧持续时间
    ...
    def bomb_update(self):
        """爆炸动画"""
        self.bomb_ticks += 1
        if self.bomb_ticks % self.bomb_everyticktime == 0:
            try:
